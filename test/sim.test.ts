...
it("step saftır: girdi state'i değişmez", () => {
...
it("oyuncular arena sınırları içinde kalır", () => {
...
it("karşılıklı basınçta oyuncular birbirinin içinden geçmez", () => {
  // P1 sağa, P2 sola: tam kafa kafaya bastırırlar.
  const s = run(() => [RIGHT, LEFT], 300);
  const [a, b] = s.players;
  const dx = a.x - b.x;
  const dy = a.y - b.y;
  const minDist = RADIUS * 2 - fromInt(1); // 1 px tolerans
  expect(dx * dx + dy * dy).toBeGreaterThan(minDist * minDist);
});

it("hashState 32 bit işaretsiz tamsayı döndürür", () => {
